...
void UUI_SlotElement::StartUpdateCD(float InCD)
{
	UUI_SlotElement::StartUpdateCD();
}

void UUI_SlotElement::StartUpdateCD()
{
	SlotIcon->SetIsEnabled(false);
}

void UUI_SlotElement::EndUpdateCD()
{
	SlotIcon->SetIsEnabled(true);
}
...
void UUI_SlotElement::SetIconSize(const FVector2D& InSize)
{
	if (SlotIcon)
	{
		SlotIcon->SetDesiredSizeOverride(InSize);
	}
}
