...
void UAnimInstanceComponentBase::ResetAxisSpeed(float InNewSpeed)
{
	if (AMMOARPGCharacterBase* InCharacterBase = Cast<AMMOARPGCharacterBase>(TryGetPawnOwner())) {
		...
		if (UCharacterMovementComponent* InCharacterMovementComponent = Cast<UCharacterMovementComponent>(InCharacterBase->GetMovementComponent())) {
			...
			/* 将最大速度归一化映射到(-1,1),方便动态的模拟Blendspace动画.*/
			AxisSpeed.X = FMath::GetMappedRangeValueClamped(FVector2D(-InNewSpeed, InNewSpeed), FVector2D(-1.f, 1.f), SpeedVector.X);
			AxisSpeed.Y = FMath::GetMappedRangeValueClamped(FVector2D(-InNewSpeed, InNewSpeed), FVector2D(-1.f, 1.f), SpeedVector.Y);
			AxisSpeed.Z = FMath::GetMappedRangeValueClamped(FVector2D(-InNewSpeed, InNewSpeed), FVector2D(-1.f, 1.f), SpeedVector.Z);
		}
	}
}
